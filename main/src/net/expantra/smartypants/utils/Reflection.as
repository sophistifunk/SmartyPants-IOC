package net.expantra.smartypants.utils
{
    ...
    public class Reflection
    {
        ...
        {
            const simpleTypes:Array = [getQualifiedClassName(String), getQualifiedClassName(Number), getQualifiedClassName(Boolean), getQualifiedClassName(uint),
        ...
        {
            return "@uri" in propertyDescription ? new QName(propertyDescription.@uri, propertyDescription.@name) : String(propertyDescription.
                                                                                                                           @name);
        }
        ...
        {
            return metaDataName ? (propertyDescription..metadata.(attribute("name") == metaDataName)) : (propertyDescription..metadata);
        }
        ...
        public static function classExtendsOrImplements(classOrClassName:*, superclass:Class):Boolean
        {
            ...
            return (factoryDescription.children().(name() == "implementsInterface" || name() == "extendsClass").(attribute("type") == superclassName).
                length() > 0)
        }
        ...
        public static function getArrayElementType(instanceOrClass:Object, fieldName:String):Class
        {
            ...
            catch (e:Error)
            {
                ...
                {
                    throw new Error("The type \"" + (aetMetadata.child("arg").attribute("value")[0]) + "\" was listed as" + " [ArrayElementType()] for " +
                                    getQualifiedClassName(instanceOrClass) + "." + fieldName + " but could not be found at run time. Make sure it is correctly spelled, fully-qualified, and being compiled.");
                }
                else
                {
                    throw e;
        ...
        {
            return sp_describeType(instanceOrClass).typeDescription..*.((name() == "variable" || name() == "accessor") && attribute("name") ==
                fieldName)[0];
        }
